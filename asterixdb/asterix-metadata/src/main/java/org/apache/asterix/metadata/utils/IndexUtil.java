...
import org.apache.asterix.codegen.asterix.column.executor.ColumnCodeGenerationExecutorFactory;
...
public class IndexUtil {
    ...
    public static ITupleProjectorFactory createTupleProjectorFactory(DatasetFormatInfo datasetFormatInfo,
            IProjectionInfo<?> projectionInfo, IProjectionInfo<?> metaProjectionInfo, ARecordType datasetType,
            ARecordType metaItemType, int numberOfPrimaryKeys) {
        if (datasetFormatInfo.getFormat() == DatasetFormat.ROW) {
            return NoOpTupleProjectorFactory.INSTANCE;
        }

        DataProjectionInfo dataProjectionInfo = (DataProjectionInfo) projectionInfo;
        DataProjectionInfo metaDataProjectionInfo = (DataProjectionInfo) metaProjectionInfo;
    ...
    public static ICodeGenerationExecutorFactory createCodeGenerationExecutorFactory(IProjectionInfo<?> projectionInfo,
            ARecordType datasetType, int numberOfPrimaryKeys) {
        CodeGenerationProjectionInfo codeGenInfo = (CodeGenerationProjectionInfo) projectionInfo;
        return new ColumnCodeGenerationExecutorFactory(datasetType, numberOfPrimaryKeys,
                codeGenInfo.getProjectionInfo(), codeGenInfo.getFunctionCallInfoMap(), codeGenInfo.getFunctionName(),
                codeGenInfo.getCode());
    }
