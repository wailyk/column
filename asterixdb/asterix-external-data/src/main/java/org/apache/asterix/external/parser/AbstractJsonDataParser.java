...
public abstract class AbstractJsonDataParser extends AbstractNestedDataParser<ADMToken> {
    ...
    private final IValueReference uidFieldName;
     ...
     * @param jsonFactory Jackson JSON parser factory.
     */
    public AbstractJsonDataParser(ARecordType recordType, JsonFactory jsonFactory, long startUid) {
        ...
        try {
            uidFieldName = parserContext.getSerializedFieldName("uid");
    ...
    protected final void parseObject(ARecordType recordType, DataOutput out) throws IOException {
        ...
        if (recordType == rootType && startUid > -1) {
            addUUID(nullBitMap, objectBuilder, valueBuffer);
        }
    ...
    private void addUUID(BitSet nullBitmap, IARecordBuilder objectBuilder, IMutableValueStorage valueBuffer)
            throws HyracksDataException {
        valueBuffer.reset();
        aInt64.setValue(startUid++);
        int64Serde.serialize(aInt64, valueBuffer.getDataOutput());
        objectBuilder.addField(0, valueBuffer);
        nullBitmap.set(0);
    }
