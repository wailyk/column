...
public class JSONDataParserFactory extends AbstractRecordStreamParserFactory<char[]> {
    ...
    private JSONDataParser createParser() {
        return new JSONDataParser(recordType, jsonFactory, getUIDStart());
    }
    ...
    private long getUIDStart() {
        if (configuration.containsKey("uidStart")) {
            return Long.parseLong(configuration.get("uidStart"));
        }
        return -1;
    }
