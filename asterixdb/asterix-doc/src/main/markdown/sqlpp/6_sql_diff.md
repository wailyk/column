...
SQL++ offers the following additional features beyond SQL-92:
  ...
  * Powerful `GROUP BY`: In addition to a set of aggregateNode functions as in standard SQL, the groups created by the `GROUP BY` clause are directly usable in nested queries and/or to obtain nested results.
