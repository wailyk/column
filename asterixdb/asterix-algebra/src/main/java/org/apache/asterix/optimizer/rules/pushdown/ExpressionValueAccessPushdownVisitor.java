...
public class ExpressionValueAccessPushdownVisitor implements ILogicalExpressionReferenceTransform {
    ...
    protected void unregisterVariableIfNeeded(LogicalVariable producedVariable, LogicalVariable variable) {
        if (builder.isVariableRegistered(variable)) {
            builder.assignVarToVar(producedVariable, variable);
            builder.unregisterVariable(variable);
        }

    }

    private LogicalVariable getProducedVariable() {
        return producedVariables != null && producedVariableIndex < producedVariables.size()
                ? producedVariables.get(producedVariableIndex) : null;
    }

    private LogicalVariable getNextProducedVariable() {
        LogicalVariable variable = getProducedVariable();
    ...
    private void pushValueAccessExpression(Mutable<ILogicalExpression> exprRef, LogicalVariable producedVar,
        ...
        if (skipPushdown(producedVar, expr)) {
            return;
        }
        ...
        //Check nested arguments if contain any pushable value access
     ...
     */
    private boolean skipPushdown(LogicalVariable producedVariable, ILogicalExpression expr) {
        if (expr.getExpressionTag() == LogicalExpressionTag.VARIABLE) {
            LogicalVariable variable = VariableUtilities.getVariable(expr);
            unregisterVariableIfNeeded(producedVariable, variable);
            return true;
        }
