...
public class OperatorValueAccessPushdownVisitor implements ILogicalOperatorVisitor<Void, Void> {
    ...
    protected final Map<LogicalVariable, AbstractScanOperator> registeredDatasets;
    ...
    public OperatorValueAccessPushdownVisitor(IOptimizationContext context) {
        this(context, new ExpectedSchemaBuilder());
    }

    public OperatorValueAccessPushdownVisitor(IOptimizationContext context, ExpectedSchemaBuilder builder) {
    ...
    public void finish() {
        for (Map.Entry<LogicalVariable, AbstractScanOperator> entry : registeredDatasets.entrySet()) {
            ...
            if (abstractScan.getOperatorTag() == LogicalOperatorTag.DATASOURCESCAN) {
                ...
                scan.setDatasetProjectionInfo(builder.createProjectionInfo(entry.getKey()));
            } else {
                UnnestMapOperator unnest = (UnnestMapOperator) abstractScan;
                unnest.setDatasetProjectionInfo(builder.createProjectionInfo(entry.getKey()));
            }
        }
     ...
     */
    public void visitInputs(ILogicalOperator op, List<LogicalVariable> producedVariables) throws AlgebricksException {
    ...
    public Void visitGroupByOperator(GroupByOperator op, Void arg) throws AlgebricksException {
        List<LogicalVariable> variableList =
                op.getGroupByList().stream().map(Pair::getFirst).collect(Collectors.toList());
        visitInputs(op, variableList);
        visitSubplans(op.getNestedPlans());
        return null;
    }
