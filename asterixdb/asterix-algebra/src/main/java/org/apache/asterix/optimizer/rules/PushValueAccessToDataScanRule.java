...
/**
 * Pushes value access expressions to dataset scan to minimize the size of the record.
 * This rule currently does not remove the value access expression. Instead, it adds the requested field names to
 * dataset details (for external datasets) or {@link ITupleProjector} (for internal datasets) to produce records that
 * only contain the requested values. Thus, no changes would occur to the plan's structure after firing this rule.
 * Example:
 * Before plan:
 * ...
