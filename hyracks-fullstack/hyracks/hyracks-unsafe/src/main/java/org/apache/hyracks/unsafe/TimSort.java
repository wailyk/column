...
final class TimSort {
    ...
    private final UnsafeSortDataFormat s;

    public TimSort(UnsafeSortDataFormat sortDataFormat) {
     ...
     * @author Josh Bloch
     */
    public void sort(LongArray a, int lo, int hi, Comparator<RecordPointerAndKeyPrefix> c) {
        assert c != null;
    ...
    private void binarySort(LongArray a, int lo, int hi, int start, Comparator<RecordPointerAndKeyPrefix> c) {
        assert lo <= start && start <= hi;
        if (start == lo)
            start++;

        RecordPointerAndKeyPrefix key0 = s.newKey();
        RecordPointerAndKeyPrefix key1 = s.newKey();

        LongArray pivotStore = s.allocate(1);
        for (; start < hi; start++) {
            s.copyElement(a, start, pivotStore, 0);
            RecordPointerAndKeyPrefix pivot = s.getKey(pivotStore, 0, key0);
     ...
     * the specified array
     */
    private int countRunAndMakeAscending(LongArray a, int lo, int hi, Comparator<RecordPointerAndKeyPrefix> c) {
        assert lo < hi;
        int runHi = lo + 1;
        if (runHi == hi)
            return 1;
     ...
     */
    private void reverseRange(LongArray a, int lo, int hi) {
        hi--;
        while (lo < hi) {
            s.swap(a, lo, hi);
            lo++;
    ...
    private class SortState {
         ...
         */
        private final Comparator<RecordPointerAndKeyPrefix> c;
         ...
         */
        private LongArray tmp; // Actual runtime type will be Object[], regardless of T
         ...
         */
        private SortState(LongArray a, Comparator<RecordPointerAndKeyPrefix> c, int len) {
            this.aLength = len;
            this.a = a;
            this.c = c;
         ...
         * should follow it.
         */
        private int gallopLeft(RecordPointerAndKeyPrefix key, LongArray a, int base, int len, int hint,
                Comparator<RecordPointerAndKeyPrefix> c) {
         ...
         * @return the int k,  0 <= k <= n such that a[b + k - 1] <= key < a[b + k]
         */
        private int gallopRight(RecordPointerAndKeyPrefix key, LongArray a, int base, int len, int hint,
                Comparator<RecordPointerAndKeyPrefix> c) {
            assert len > 0 && hint >= 0 && hint < len;
        ...
        private void mergeLo(int base1, int len1, int base2, int len2) {
            ...
            LongArray a = this.a; // For performance
            LongArray tmp = ensureCapacity(len1);
            ...
            Comparator<RecordPointerAndKeyPrefix> c = this.c; // Use local variable for performance
        ...
        private void mergeHi(int base1, int len1, int base2, int len2) {
            ...
            LongArray a = this.a; // For performance
            LongArray tmp = ensureCapacity(len2);
         ...
         */
        private LongArray ensureCapacity(int minCapacity) {
