...
public class LogicalOperatorPrettyPrintVisitor extends AbstractLogicalOperatorPrettyPrintVisitor<Integer>
    ...
    public Void visitUnnestMapOperator(UnnestMapOperator op, Integer indent) throws AlgebricksException {
        ...
        appendProjectInformation(plan, "project-dataset", op.getDatasetProjectionInfo(), indent);
        appendProjectInformation(plan, "project-meta", op.getMetaProjectionInfo(), indent);
        return null;
    }
    ...
    private void appendProjectInformation(AlgebricksStringBuilderWriter plan, String projectionSource,
            IProjectionInfo<?> projectionInfo, int indent) {
        final String projectedFields = projectionInfo == null ? "" : projectionInfo.toString();
        if (projectedFields.isEmpty()) {
            return;
        }

        ProjectorType projectorType = projectionInfo.getProjectorType();
        plan.append(" ");
        if (projectorType == ProjectorType.COMPILED) {
            plan.append("code >>");
        } else {
        ...
        if (projectorType == ProjectorType.COMPILED) {
            appendMultiLine(projectedFields, indent);
        } else {
            plan.append(" (");
    ...
    private void appendMultiLine(String value, int indent) {
        String[] lines = value.split("\n");
        buffer.append("\n");
        String format = "%0" + String.valueOf(lines.length).length() + "d";
        for (int i = 0; i < lines.length; i++) {
            String line = lines[i];
            String lineNumber = String.format(format, i + 1);
            addIndent(indent).append(lineNumber).append("| ").append(line).append("\n");
        }
    }
}
